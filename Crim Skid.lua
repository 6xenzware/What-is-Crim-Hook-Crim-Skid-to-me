		...
		function Sections:NewSection(secName, hidden)
			...
			function Elements:NewDropdown(dropname, dropinf, list, callback)
				...
				for i, v in next, list do
					...
					optionSelect.MouseEnter:Connect(function()
						if not focusing then
							game.TweenService
								:Create(optionSelect, TweenInfo.new(0.1, Enum.EasingStyle.Linear, Enum.EasingDirection.In), {
									BackgroundColor3 = Color3.fromRGB(themeList.ElementColor.r * 255 + 8, themeList.ElementColor.g * 255 + 9, themeList.ElementColor.b * 255 + 10),
								})
								:Play()
							oHover = true
			...
			function Elements:NewColorPicker(colText, colInf, defcolor, callback)
				...
				local function togglerainbow()
					if rainbow then
						game.TweenService
							:Create(toggleEnabled, TweenInfo.new(0.1, Enum.EasingStyle.Linear, Enum.EasingDirection.InOut), {
								ImageTransparency = 1,
							})
							:Play()
						rainbow = false
	...
	swimbeat = RunService.Heartbeat:Connect(function()
		pcall(function()
			if character and rootPart then
				...
				rootPart.Velocity = (moveDirection ~= Vector3.new() or UserInputService:IsKeyDown(Enum.KeyCode.Space)) and rootPart.Velocity or Vector3.new() -- Prevents character from falling
			end
		end)
	end)
end
...
getgenv().PredictionVelocity = 10 -- The speed of the PredictMovement feature 
	...
	getgenv().Notify = function(title, text, icon, time)
		if CanNotify == true then
			if not time or not type(time) == "number" then
				time = 3
			end
			SGui:SetCore("SendNotification", {
				Title = title,
...
Section5:NewButton("Enable Legit Aimbot", "[Y]: Enables legit aimbot", function()
	...
	RunService.RenderStepped:Connect(function()
		if Aimlock and MousePressed and AimlockTarget and AimlockTarget.Character and AimlockTarget.Character:FindFirstChild(getgenv().AimPart) then
			...
			aimPos = aimPos + Vector3.new(math.random() * getgenv().JitterAmount - getgenv().JitterAmount / 2, math.random() * getgenv().JitterAmount - getgenv().JitterAmount / 2, math.random() * getgenv().JitterAmount - getgenv().JitterAmount / 2)
